...
const Session = require("../superadmin/session.model");
...
function hashRefreshTokenSecret(tokenSecret) {
  return crypto
    .createHash("sha256")
    .update(`${securityConfig.superadminJwtSecret}:${String(tokenSecret || "")}`)
    .digest("hex");
}
...
function getSessionExpiryDate() {
  return new Date(Date.now() + securityConfig.superadminRefreshTokenTtlMs);
}
...
function assertSessionUsable(session) {
  if (!session) {
    throw AppError.unauthorized("Superadmin refresh session was not found");
  }

  if (session.status !== "active" || session.revokedAt) {
    throw AppError.unauthorized("Superadmin refresh session is no longer active");
  }

  if (new Date(session.expiresAt).getTime() <= Date.now()) {
    throw AppError.unauthorized("Superadmin refresh session has expired");
  }

  return session;
}

async function createRefreshSession({ superadminId, ...metadata } = {}) {
...
async function rotateRefreshSession(refreshToken, metadata = {}) {
  ...
  if (session.deviceId && normalizedMetadata.deviceId && session.deviceId !== normalizedMetadata.deviceId) {
    throw AppError.unauthorized("Superadmin refresh session device mismatch");
  }
...
async function revokeSessionById(sessionId, { superadminId, reason = "revoked" } = {}) {
...
async function revokeAllSuperadminSessions(superadminId, reason = "revoked") {
  const now = new Date();
  const result = await Session.updateMany(
    { superadminId, status: "active" },
    {
      $set: {
        status: "revoked",
        revokedAt: now,
...
async function revokeOldestActiveSessions(superadminId, count = 1, reason = "session_limit_rebalanced") {
  const limit = Math.max(0, Number(count) || 0);
  if (!superadminId || limit <= 0) {
    return 0;
  }
...
async function assertSessionIsActive(sessionId, superadminId = null) {
  if (!sessionId) {
    return null;
  }
...
async function countActiveSessions(superadminId) {
