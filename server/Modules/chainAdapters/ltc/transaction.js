...
const DUST_THRESHOLD_LITOSHI = 546n;
...
function normalizeAddress(address, label, network) {
  try {
    ...
    throw AppError.validation(`Invalid LTC ${label}`);
  }
}
...
function normalizeExecutionParams(input = {}) {
  ...
  if (unsupportedKeys.length) {
    throw AppError.validation(
      `LTC transfers do not support execution params: ${unsupportedKeys.join(", ")}`,
    );
  }
...
async function fetchSpendableUtxos(input = {}) {
  const normalizedNetwork = client.normalizeNetwork(input.network);
  const address = normalizeAddress(input.address, "address", normalizedNetwork);
  const ltcClient = client.getClient(normalizedNetwork);
  const utxos = await ltcClient.fetchAddressUtxos(address);

  return normalizeUtxos(Array.isArray(utxos) ? utxos : []);
}
...
function selectUtxos(utxos, recipientAmountBaseUnits, feeRate) {
  ...
  if (!availableUtxos.length) {
    logger.warn("LTC UTXO selection failed because no confirmed spendable inputs were found", {
      feeRate,
    });
    throw AppError.validation("No confirmed LTC UTXOs are available for this wallet");
  }
  ...
  for (const utxo of availableUtxos) {
    ...
    if (tentativeChange >= DUST_THRESHOLD_LITOSHI) {
  ...
  logger.warn("LTC UTXO selection failed due to insufficient confirmed balance after fee calculation", {
    feeRate,
  ...
  throw AppError.validation("Insufficient LTC balance to cover the amount and network fee");
}
...
function extractErrorMessage(error, fallback = "Litecoin request failed") {
  const candidates = [
    error?.response?.data,
    error?.data?.message,
    error?.message,
...
function buildRpcError(error, fallbackMessage) {
  ...
  if (
    ...
    return AppError.validation("Insufficient LTC balance to cover the amount and network fee");
  }
  ...
  if (
    ...
    return AppError.validation("Invalid LTC destination address");
  }
  ...
  if (
    ...
    return AppError.conflict("The selected LTC inputs are no longer available. Please try again");
  }
  ...
  if (
    ...
    return AppError.conflict("This LTC wallet already has too many chained pending transactions");
  }
  ...
  if (
    ...
    return new AppError("Litecoin provider is currently unavailable", {
      status: 502,
      errors: { reason },
    });
  }
...
function assertNativeLtcAsset(input = {}) {
  const asset = String(input.asset || input.assetDescriptor?.asset || "LTC").trim().toUpperCase();
  const assetType = String(input.assetDescriptor?.assetType || "native").trim().toLowerCase();

  if (asset !== "LTC" || assetType !== "native") {
    throw AppError.validation("LTC adapter only supports native LTC transfers");
  }
}
...
async function resolveDedicatedChangeAddress(network, fromAddress, mnemonic, { persist = false } = {}) {
  ...
  if (!changeAddress) {
    throw AppError.internal("Failed to derive LTC change address");
  }
  ...
  logger.info("Created dedicated LTC change address", {
...
async function buildPreparedTransfer(network, input, options = {}) {
  ...
  if (fromAddress === toAddress) {
    throw AppError.validation("Cannot send LTC to the same wallet address");
  }
  ...
  if (recipientAmountBaseUnits <= 0n) {
    throw AppError.validation("LTC amount must be greater than 0");
  }

  if (recipientAmountBaseUnits < DUST_THRESHOLD_LITOSHI) {
    throw AppError.validation(
      `LTC amount must be at least ${amount.fromBaseUnits(DUST_THRESHOLD_LITOSHI.toString())} LTC`,
    );
  }
  ...
  if (
    ...
    throw AppError.validation("Unable to resolve a valid LTC change address");
  }
...
function buildPsbt(network, prepared, signingAddress, signer) {
  const litecoinNetwork = wallet.getLitecoinNetwork(network);
  const sourceScript = bitcoin.address.toOutputScript(signingAddress, litecoinNetwork);
...
async function estimateTransfer(input) {
  try {
    ...
    throw buildRpcError(error, "Failed to estimate LTC transfer");
  }
}
...
async function assertPreviewTransferAllowed({ preview }) {
  ...
  if (amountBaseUnits < DUST_THRESHOLD_LITOSHI) {
    throw AppError.validation(
      `LTC amount must be at least ${amount.fromBaseUnits(DUST_THRESHOLD_LITOSHI.toString())} LTC`,
    );
  }
  ...
  if (networkFeeBaseUnits <= 0n) {
    throw new AppError("Unable to determine LTC network fee", {
      status: 502,
      errors: {
        reason: "Litecoin fee estimate returned a zero network fee",
      },
    });
  }
}
...
async function executeTransfer(input) {
  try {
    ...
    if (!privateKey || !privateKey.length) {
      throw AppError.conflict("Derived LTC signing key is unavailable");
    }

    const signer = ECPair.fromPrivateKey(privateKey, {
      network: wallet.getLitecoinNetwork(normalizedNetwork),
    });
    ...
    const ltcClient = client.getClient(normalizedNetwork);
    const broadcastTxHash = await ltcClient.broadcastTransaction(rawHex);
    const txHash = broadcastTxHash || localTxHash;
    const submittedTransaction = await ltcClient.fetchTransaction(txHash).catch(() => null);
    const tipHeight =
      submittedTransaction?.status?.confirmed === true
        ? await ltcClient.fetchTipHeight().catch(() => 0)
        : 0;
    ...
    throw buildRpcError(error, "Failed to submit LTC transfer");
  }
}
...
async function fetchHistory(input = {}) {
  ...
  try {
    const ltcClient = client.getClient(normalizedNetwork);
    const tipHeightPromise = ltcClient.fetchTipHeight().catch(() => 0);
    const entries = [];
    const seen = new Set();
    let batch = await ltcClient.fetchAddressTransactions(address);
    ...
    while (Array.isArray(batch) && batch.length && entries.length < limit) {
      ...
      batch = await ltcClient.fetchAddressTransactionsChain(address, lastSeenTxId);
    }
    ...
    logger.warn("Failed to fetch LTC history from provider", {
      address,
      network: normalizedNetwork,
      limit,
