...
const DEFAULT_NETWORK = String(process.env.LTC_DEFAULT_NETWORK || "mainnet").trim().toLowerCase();
...
function normalizeNetwork(network = DEFAULT_NETWORK) {
  ...
  if (normalized !== "mainnet") {
    throw AppError.validation(`Unsupported LTC network "${network}"`);
  }
...
function ensureHttpUrl(url, fieldName) {
  ...
  if (!normalized) {
    throw AppError.validation(`${fieldName} is required for Litecoin operations`);
  }
...
function getBaseUrl(network = DEFAULT_NETWORK) {
  ...
  if (normalizedNetwork === "mainnet") {
    return ensureHttpUrl(
      process.env.LTC_MAINNET_API_URL,
      "LTC_MAINNET_API_URL",
    );
  }

  throw AppError.validation(`Unsupported LTC network "${normalizedNetwork}"`);
}
...
async function request(network, path, options = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        body
          ? `LTC API HTTP ${response.status}: ${body}`
          : `LTC API HTTP ${response.status}`,
      );
    }
    ...
    throw new Error(
      `Failed LTC API request for ${normalizedNetwork}: ${error instanceof Error ? error.message : String(error)}`,
    );
  } finally {
