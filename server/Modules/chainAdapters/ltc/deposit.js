...
function normalizeAddress(address, network) {
  try {
    ...
    throw AppError.validation("LTC deposit address is invalid");
  }
}
...
function buildCanonicalDepositEntry(rawTx, output, outputIndex, targetAddress) {
  const txHash = String(rawTx?.txid || "").trim();
  const indexedOutput = Number.isInteger(Number(outputIndex)) ? Number(outputIndex) : null;
  const vout = Number.isInteger(Number(output?.n)) ? Number(output.n) : indexedOutput;
    ...
    .flatMap((entry) =>
      (Array.isArray(entry?.vout) ? entry.vout : [])
        ...
        .map(({ output, outputIndex }) =>
          buildCanonicalDepositEntry(entry, output, outputIndex, targetAddress),
        ),
    )
    .filter((entry) => {
      if (!entry) {
