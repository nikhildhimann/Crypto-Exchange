...
function normalizeAddress(address, label) {
  ...
  if (!wallet.validateAddress(normalized)) {
    throw AppError.validation(`Invalid ETH ${label}`);
  }

  return getAddress(normalized);
}

function extractErrorMessage(error, fallback = "Ethereum request failed") {
  const candidates = [
    error?.shortMessage,
...
function buildRpcError(error, fallbackMessage) {
  ...
  if (normalizedReason.includes("insufficient funds")) {
    return AppError.validation("Insufficient ETH balance to cover the amount and network fee");
  }

  if (normalizedReason.includes("nonce too low")) {
    return AppError.conflict("ETH transaction nonce is no longer valid. Please try again");
  }
  ...
  if (
    ...
    return AppError.conflict("A conflicting ETH transaction is already pending for this wallet");
  }

  if (normalizedReason.includes("already known")) {
    return AppError.conflict("This ETH transaction is already known by the network");
  }
  ...
  if (
    ...
    return new AppError("Ethereum RPC is currently unavailable", {
      status: 502,
      errors: { reason },
    });
  }
...
function getExplorerApiConfig() {
  return {
    url: String(process.env.ETH_EXPLORER_API_URL || "").trim(),
    apiKey: String(process.env.ETH_EXPLORER_API_KEY || "").trim(),
  };
}
...
function normalizeExecutionParams(input = {}) {
  ...
  if (unsupportedKeys.length) {
    throw AppError.validation(
      `ETH native transfers do not support execution params: ${unsupportedKeys.join(", ")}`,
    );
  }
  ...
  if (gasPrice !== undefined && (maxFeePerGas !== undefined || maxPriorityFeePerGas !== undefined)) {
    throw AppError.validation(
      "Use either gasPrice or maxFeePerGas/maxPriorityFeePerGas for ETH transfers, not both",
    );
  }
...
function buildBaseTransferRequest(input) {
  ...
  if (valueBaseUnits <= 0n) {
    throw AppError.validation("ETH amount must be greater than 0");
  }
...
async function resolveGasLimit(provider, transactionRequest, executionParams) {
  ...
  try {
    ...
    throw buildRpcError(error, "Failed to estimate ETH gas limit");
  }
}
...
async function resolveFeeOverrides(provider, executionParams, gasLimit) {
  ...
  if (canUseEip1559) {
    ...
    if (maxFeePerGas === null) {
      throw AppError.internal("Unable to determine ETH maxFeePerGas");
    }
  ...
  if (resolvedFeeData.gasPrice === null) {
    throw new AppError("Unable to determine ETH network fee", {
      status: 502,
      errors: {
        reason: "Ethereum RPC did not return gas price or EIP-1559 fee data",
      },
    });
  }
...
async function estimateTransfer(input) {
  try {
    ...
    throw buildRpcError(error, "Failed to estimate ETH transfer");
  }
}
...
async function assertPreviewTransferAllowed({
  ...
  if (networkFeeBaseUnits <= 0n) {
    throw new AppError("Unable to determine ETH network fee", {
      status: 502,
      errors: {
        reason: "Ethereum gas estimate returned a zero network fee",
      },
    });
  }
  ...
  if (amountBaseUnits < networkFeeBaseUnits) {
    throw AppError.validation(
      `ETH amount is too small relative to the current network fee (${fromBaseUnits(networkFeeBaseUnits)} ETH)`,
    );
  }
}
...
async function executeTransfer(input) {
  try {
    ...
    throw buildRpcError(error, "Failed to submit ETH transfer");
  }
}
...
async function fetchHistory() {
  ...
  if (!explorerConfig.url) {
    logger.warn("Skipping ETH history fetch because ETH_EXPLORER_API_URL is not configured", {
      address,
      network,
    });
    return [];
  ...
  if (!wallet.validateAddress(address)) {
    logger.warn("Skipping ETH history fetch because the wallet address is invalid", {
      address,
      network,
    });
    return [];
  ...
  try {
    ...
    logger.warn("Failed to fetch ETH history from explorer", {
      address,
      network,
      limit,
