...
const POLYGON_DERIVATION_PATH = "m/44'/60'/0'/0/0";
...
function deriveWalletFromMnemonic(mnemonic) {
  ...
  const wallet = HDNodeWallet.fromPhrase(normalizedMnemonic, "", POLYGON_DERIVATION_PATH);
...
function normalizePrivateKey(secret) {
  ...
  throw AppError.validation("Invalid Polygon wallet secret");
}
