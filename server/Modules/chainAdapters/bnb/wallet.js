...
const BNB_DERIVATION_PATH = "m/44'/60'/0'/0/0";
...
function deriveWalletFromMnemonic(mnemonic) {
  ...
  const wallet = HDNodeWallet.fromPhrase(normalizedMnemonic, "", BNB_DERIVATION_PATH);
...
function normalizePrivateKey(secret) {
  ...
  throw AppError.validation("Invalid BNB wallet secret");
}
