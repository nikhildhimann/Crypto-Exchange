...
const SORT_FIELDS = new Set(["createdAt", "updatedAt", "action", "resource", "status"]);
...
function parseQuery(searchParams) {
  ...
  return {
    ...
    action: normalizeLowerString(searchParams.get("action")),
    resource: normalizeLowerString(searchParams.get("resource")),
...
export function useSuperadminAuditQueryState() {
  ...
  const [resourceDraft, setResourceDraft] = useState(query.resource);
  ...
  useEffect(() => {
    setResourceDraft(query.resource);
  }, [query.resource]);
  ...
  return {
    ...
    applyActionFilter: () =>
      updateParams({ action: normalizeLowerString(actionDraft) }, { replace: true, resetPage: true }),
    clearActionFilter: () => {
      setActionDraft("");
      updateParams({ action: "" }, { replace: true, resetPage: true });
    },
    applyResourceFilter: () =>
      updateParams({ resource: normalizeLowerString(resourceDraft) }, { replace: true, resetPage: true }),
    clearResourceFilter: () => {
      setResourceDraft("");
      updateParams({ resource: "" }, { replace: true, resetPage: true });
    },
    openAuditEvent: (auditId) => updateParams({ selected: auditId }, { replace: true }),
    closeAuditEvent: () => updateParams({ selected: "" }, { replace: true }),
    clearFilters: () =>
      updateParams(
        {
          search: "",
