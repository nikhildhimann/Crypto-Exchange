...
const DEFAULT_SORT_BY = "jobName";
const DEFAULT_SORT_ORDER = "asc";
const SORT_FIELDS = new Set(["jobName", "status", "intervalMs"]);
...
function countActiveFilters(query) {
  return [query.search, query.status, query.enabled].filter(Boolean).length;
}

export function useSuperadminJobsQueryState() {
  ...
  return {
    ...
    openJob: (selected) => updateParams({ selected }, { replace: true }),
    closeJob: () => updateParams({ selected: "" }, { replace: true }),
    clearFilters: () =>
      updateParams(
        {
          search: "",
