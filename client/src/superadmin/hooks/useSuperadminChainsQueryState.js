...
const SORT_FIELDS = new Set(["code", "label", "family", "runtimeStatus"]);
...
function parseQuery(searchParams) {
  ...
  return {
    ...
    family: normalizeLowerString(searchParams.get("family")),
    status: normalizeLowerString(searchParams.get("status")),
    enabled: normalizeBooleanQuery(searchParams.get("enabled")),
    sortBy: normalizeSortBy(searchParams.get("sortBy")),
    sortOrder: normalizeSortOrder(searchParams.get("sortOrder")),
    selected: normalizeLowerString(searchParams.get("selected")),
  };
}

function countActiveFilters(query) {
  return [query.search, query.family, query.status, query.enabled].filter(Boolean).length;
}

export function useSuperadminChainsQueryState() {
  ...
  return {
    ...
    setFamily: (family) => updateParams({ family: normalizeLowerString(family) }, { replace: true, resetPage: true }),
    setStatus: (status) => updateParams({ status: normalizeLowerString(status) }, { replace: true, resetPage: true }),
    setEnabled: (enabled) =>
      updateParams({ enabled: normalizeBooleanQuery(enabled) }, { replace: true, resetPage: true }),
    setSort: ({ sortBy, sortOrder }) => updateParams({ sortBy, sortOrder }, { replace: true, resetPage: true }),
    openChain: (selected) => updateParams({ selected: normalizeLowerString(selected) }, { replace: true }),
    closeChain: () => updateParams({ selected: "" }, { replace: true }),
    clearFilters: () =>
      updateParams(
        {
          search: "",
