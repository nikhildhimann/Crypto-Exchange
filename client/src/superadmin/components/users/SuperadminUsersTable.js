  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: 8 }).map((_, index) => (
        <div
          key={index}
          className="grid grid-cols-[1.6fr_0.8fr_0.8fr_0.8fr_0.8fr_1fr_1fr_0.7fr] gap-3 rounded-[20px] border border-white/8 bg-slate-950/45 px-4 py-4"
        >
...
function EmptyState() {
  return (
    <div className="rounded-[24px] border border-dashed border-white/12 bg-slate-950/40 px-5 py-8 text-center">
      <p className="text-base font-semibold text-white">No users matched these filters</p>
...
function ErrorState({ message, onRetry }) {
  return (
    <div className="rounded-[24px] border border-rose-300/15 bg-rose-400/10 px-5 py-6">
      <p className="text-base font-semibold text-rose-50">Users list unavailable</p>
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          <p className="text-sm font-semibold text-white">{formatUserDisplayId(item)}</p>
          <p className="text-xs leading-5 text-slate-400">
            {item.primaryChain ? String(item.primaryChain).toUpperCase() : "Primary chain unavailable"}
          </p>
        </div>
        <div className="flex flex-col items-end gap-2">
          <SuperadminUsersBadge tone={getUserStatusTone(item.status)}>
            {humanizeUserValue(item.status)}
          </SuperadminUsersBadge>
          <SuperadminUsersBadge tone={getUserRoleTone(item.role)}>
            {humanizeUserValue(item.role || "user")}
          </SuperadminUsersBadge>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-xs text-slate-400">
        <div>
          ...
          <p className="mt-1 text-slate-200">{item.mfaEnabled ? "Enabled" : "Disabled"}</p>
        </div>
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">
            Last access
          </p>
          <p className="mt-1 text-slate-200">{formatUserRelativeTime(item.lastAccessAt)}</p>
        </div>
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">
            Created
          </p>
          <p className="mt-1 text-slate-200">{formatUserRelativeTime(item.createdAt)}</p>
        </div>
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">
            Public key
          ...
          <SuperadminCopyValue
            ...
            displayValue={truncateMiddle(item.publicKey, 10, 6)}
          />
        </div>
      </div>
    </button>
  ...
  return (
    <div className="overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.03))] shadow-[0_18px_60px_rgba(2,6,23,0.24)] backdrop-blur-xl">
      <div className="flex items-center justify-between gap-4 border-b border-white/8 px-5 py-5 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold tracking-tight text-white">Users</h2>
          <p className="mt-1 text-sm leading-6 text-slate-400">
            Global user identities with server-driven filtering and independent detail inspection.
          </p>
        </div>
        {state.loading && data ? (
          <SuperadminUsersBadge tone="cyan">Updating</SuperadminUsersBadge>
        ) : null}
      </div>
          ...
          <>
              ...
              <table className="min-w-full border-separate border-spacing-y-3">
                  <thead>
                    <tr className="text-left text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">
                      ...
                      <th className="px-4 py-2">Primary chain</th>
                  ...
                  <tbody>
                    {items.map((item) => (
                      <tr key={item.id} className="rounded-[20px] bg-slate-950/45 text-sm text-slate-300">
                        <td className="rounded-l-[20px] border border-r-0 border-white/8 px-4 py-4">
                          <div className="space-y-1">
                            <SuperadminCopyValue
                              ...
                              compact
                              label="user id"
                              value={item.publicAddress || item.publicKey || item.id}
                              displayValue={truncateMiddle(item.publicAddress || item.publicKey || item.id, 12, 6)}
                              mono={false}
                            />
                          </div>
                        </td>
                        ...
                        <td className="border border-l-0 border-r-0 border-white/8 px-4 py-4 text-white">
                          {item.primaryChain ? String(item.primaryChain).toUpperCase() : "—"}
                        </td>
                        <td className="border border-l-0 border-r-0 border-white/8 px-4 py-4">
                          <SuperadminUsersBadge tone={item.mfaEnabled ? "emerald" : "slate"}>
                            {item.mfaEnabled ? "Enabled" : "Disabled"}
                          </SuperadminUsersBadge>
                        </td>
                        <td
                          className="border border-l-0 border-r-0 border-white/8 px-4 py-4"
                          title={formatUserAbsoluteTime(item.lastAccessAt)}
                        >
                          {formatUserRelativeTime(item.lastAccessAt)}
                        </td>
                        <td
                          className="border border-l-0 border-r-0 border-white/8 px-4 py-4"
                          title={formatUserAbsoluteTime(item.createdAt)}
                        >
                          {formatUserRelativeTime(item.createdAt)}
                        </td>
            ...
            <div className="mt-5 flex flex-col gap-4 border-t border-white/8 pt-5 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-slate-400">
                Showing page {data.page} of {data.totalPages || 1} with {data.total} total users.
              </p>
